...
- Projected-volume consistency polling is opt-in with a 50 ms minimum interval, bounding one
  watcher's idle stat checks to at most 20 per second. Idle ticks are metadata-only; fingerprint
  changes reuse the serialized reload queue before any snapshot read or parse.
