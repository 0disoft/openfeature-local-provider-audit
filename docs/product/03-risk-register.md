...
| Projected-volume reload gap | Kubernetes-style internal symlink swaps do not emit the visible filename and leave a stale snapshot. | Use explicit reload or consumer-owned atomic copies today; ADR 0011 and Linux fixtures define and reproduce the gap while bounded polling remains proposed. |
