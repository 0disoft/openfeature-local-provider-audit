...
- The workflow runs `stable-release-gate` before packaging. Stable `1.x` is
  rejected unless `docs/testing/cross-repository-consumer-evidence.json` contains a complete,
  maintainer-reviewed separate-repository consumer result.
...
- Stable `1.0.0` is published while the cross-repository consumer evidence record is pending,
  incomplete, same-repository, or based only on package-repository smoke coverage.
